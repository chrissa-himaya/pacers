                    ...
                    @foreach($data_items['data']->toArray() as $key => $value)
                        @if(!in_array($key, $data_items['column_hidden']))
                            <tr>
                                ...
                                <td>
                                    @if($key=='school_unit_id')
                                        <select name="school_unit_id" id="school_unit_id" class="form-control select2">
                                            <option value="">-- Select Schooling Unit --</option>
                                            @foreach($data_items['schoolingunits'] as $id => $schoolingunit)
                                                <option value="{{ $id }}"
                                                    {{ old('school_unit_id', $data_items['data']->school_unit_id ?? '') == $id ? 'selected' : '' }}>
                                                    {{ $schoolingunit }}
                                                </option>
                                            @endforeach
                                        </select>
