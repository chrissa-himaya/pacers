                        ...
                        @foreach($data_items['data']->toArray() as $key => $value)
                            @if(!in_array($key, $data_items['column_hidden']))
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="text" class="form-control" value="{{ old(
                                    $key,
                                    $data_items['operation_type'] === 'create'
                                    ? ($key === 'name' ? 'Class' : '')
                                    : $value
                                ) }}" name="{{ $key }}" @disabled($data_items["operation_type"] === "show") @if(isset($data_items['required_fields']) && in_array($key, $data_items['required_fields'])) required @endif>
                                                </td>
                                            </tr>
                            @endif
                        @endforeach
