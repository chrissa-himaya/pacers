                        ...
                        @if(!in_array($key, $data_items['column_hidden']))
                            <tr>
                                ...
                                <td>
                                    ...
                                    <select name="assignment_id" class="form-control select2" required {{$data_items["operation_type"] == "show" ? "disabled" : ""}} required>
                                        @foreach($data_items["assignments"] as $id => $assignment)
                                            <option value="{{ $id }}" {{ (isset($data_items["data"]) && $data_items["data"]->assignment_id ? $data_items["data"]->assignment_id : old('assignment_id')) == $id ? 'selected' : '' }}>
                                                {{ $assignment }}</option>
                                        @endforeach
                                    </select>
                                    
                                    @elseif($key=="rank_id")
                                    <select name="rank_id" class="form-control select2" required {{$data_items["operation_type"] == "show" ? "disabled" : ""}} required>
                                        @foreach($data_items["ranks"] as $id => $rank)
                                            <option value="{{ $id }}" {{ (isset($data_items["data"]) && $data_items["data"]->rank_id ? $data_items["data"]->rank_id : old('rank_id')) == $id ? 'selected' : '' }}>
                                                {{ $rank }}</option>
                                        @endforeach
                                    </select>
                                    @else
