    ...
    <div class="index-wrapper">
        <div class="card">
            <div class="index-header">
                <h4><i class="fas fa-heartbeat"></i>{{ $config_data->module_name }}</h4>
            ...
            <table id="dataTable" class="table table-bordered" style="width: 100% !important;">
                <thead>
                    ...
                    <tr class="filter-row">
                        ...
                        <th><input type="text" placeholder="Entry" class="form-control form-control-sm" /></th>
                        <th><input type="text" placeholder="Rating" class="form-control form-control-sm" /></th>
                        ...
                        <th><input type="text" placeholder="Unit" class="form-control form-control-sm" /></th>
                        <th></th>
                        <th></th>
                    </tr>
                </thead>
...
@section('scripts')
    <script>
        let canView    = @json(auth()->user()->can($config_data->module_perm_name.'_show', App\Models\PftHistory::class));
        let canUpdate  = @json(auth()->user()->can($config_data->module_perm_name.'_edit', App\Models\PftHistory::class));
        let canDelete  = @json(auth()->user()->can($config_data->module_perm_name.'_delete', App\Models\PftHistory::class));
        let canCreate  = @json(auth()->user()->can($config_data->module_perm_name.'_create', App\Models\PftHistory::class));
        ...
        const table = $('#dataTable').DataTable({
            ...
            columns: [
                ...
                { data: 'entry', searchable: true, defaultContent: '-' },
                {
                    data: 'rating',
                    searchable: true,
                    defaultContent: '-',
                    render: function (data) {
                        return data ? '<span class="rating-badge">' + parseFloat(data).toFixed(2) + '%</span>' : '-';
                    }
                },
                {
                    data: 'date_taken',
                ...
                { data: 'supervising_unit', searchable: true, defaultContent: '-' },
                {
                    data: 'points',
                    render: function (data) {
                        return data ? '<span class="pts-badge">' + parseFloat(data).toFixed(2) + '</span>' : '-';
                    },
                    className: 'mono'
                },
                {
                    ...
                    render: function (data) {
                        ...
                        if (canDelete) btns += `
                            <form action="/${url_route}/${data}" method="POST" style="display:inline;margin:0;">
                                ...
                                <button type="submit" class="btn btn-delete" onclick="return confirm('Delete this PFT record?')">
                                    <i class="fas fa-trash"></i>
                                </button>
                            </form>`;
                        if (canCreate) btns += `<a href="/${url_route}/${data}/add-entry" class="btn btn-add" title="Add PFT for same officer"><i class="fas fa-plus"></i> Add</a>`;
                        btns += '</div>';
                        return btns;
                    }
                }
