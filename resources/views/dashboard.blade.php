...
<style>
    ...
    *,*::before,*::after{box-sizing:border-box}
    ...
    .chart-scroll-outer{height:100%;overflow-x:auto;overflow-y:hidden;scrollbar-width:thin;scrollbar-color:rgba(30,140,74,.25) transparent}
    .chart-scroll-outer::-webkit-scrollbar{height:5px}
    .chart-scroll-outer::-webkit-scrollbar-thumb{background:rgba(30,140,74,.25);border-radius:4px}
    ...
    .chart-card-tall .card-bd{height:calc(100% - 80px);min-height:350px}
    ...
    .drill-modal{background:var(--panel);border-radius:var(--radius);box-shadow:0 20px 60px rgba(0,0,0,.2);width:90%;max-width:860px;max-height:82vh;display:flex;flex-direction:column;overflow:hidden}
    .drill-hd{padding:14px 20px;background:linear-gradient(135deg,var(--accent-light),var(--panel-2));border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;gap:10px}
    .drill-hd h5{margin:0;font-size:14px;font-weight:800;color:var(--text);flex:1}
    .drill-hd-actions{display:flex;gap:8px;align-items:center}
    ...
    .btn-print{display:flex;align-items:center;gap:5px;padding:6px 12px;border-radius:8px;border:1px solid var(--border);background:var(--panel);font-size:11px;font-weight:700;color:var(--text-2);cursor:pointer;font-family:inherit;transition:all .2s}
    .btn-print:hover{background:var(--accent-light);border-color:rgba(30,140,74,.3);color:var(--accent)}
    .btn-csv{display:flex;align-items:center;gap:5px;padding:6px 12px;border-radius:8px;border:1px solid rgba(13,148,136,.35);background:var(--teal-light);font-size:11px;font-weight:700;color:var(--teal);cursor:pointer;font-family:inherit;transition:all .2s}
    .btn-csv:hover{background:var(--teal);color:#fff;border-color:var(--teal)}
    .btn-excel{display:flex;align-items:center;gap:5px;padding:6px 12px;border-radius:8px;border:1px solid rgba(22,163,74,.35);background:var(--good-light);font-size:11px;font-weight:700;color:var(--good);cursor:pointer;font-family:inherit;transition:all .2s}
    .btn-excel:hover{background:var(--good);color:#fff;border-color:var(--good)}
    ...
    @media print {
        ...
        #printSection { position: fixed; left: 0; top: 0; width: 100%; padding: 20px; }
        .btn-print, .drill-close, .drill-hd-actions { display: none !important; }
    }
    ...
    <div class="drill-modal">
        <div class="drill-hd">
            ...
            <div class="drill-hd-actions">
                <button class="btn-csv"   onclick="downloadDrill('csv')"   title="Download as CSV"><i class="fas fa-file-csv"></i> CSV</button>
                <button class="btn-excel" onclick="downloadDrill('excel')" title="Download as Excel"><i class="fas fa-file-excel"></i> Excel</button>
                <button class="btn-print" onclick="printDrillList()" title="Print list"><i class="fas fa-print"></i> Print</button>
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane active" id="tab-overview">
            <div class="dash-grid">
                <div class="kpi-grid" style="width:300px">
                    ...
                    <div class="card kpi kpi-clickable" onclick="fetchAssignedOfficers('assigned')" title="Click to view assigned officers" style="--kpi-hover-color:rgba(22,163,74,.15)"><div class="meta"><div class="label">Assigned</div><div class="value val-green">{{ number_format($stats['assigned_officers']) }}</div><div class="sub">{{ $stats['assigned_rate'] }}% coverage</div></div><div class="icon good"><i class="fas fa-user-check"></i></div></div>
                    <div class="card kpi kpi-clickable" onclick="fetchAssignedOfficers('unassigned')" title="Click to view unassigned officers"><div class="meta"><div class="label">Unassigned</div><div class="value val-warn">{{ number_format($stats['unassigned_officers']) }}</div><div class="sub">Needs review</div></div><div class="icon warn"><i class="fas fa-user-times"></i></div></div>
                            ...
                            <thead><tr><th>PAMU</th>@foreach($pamuRanks as $pr)<th style="text-align:center">{{ $pr }}</th>@endforeach<th style="text-align:right">Total</th></tr></thead>
                                <tbody>
                                    @foreach($pamuRecap as $row)
                                        <tr>
                                            <td title="{{ $row['name'] ?? '' }}"><span class="badge-rank" style="font-family:var(--font-sans);background:var(--blue-light);color:var(--blue);border-color:rgba(37,99,235,.2)">{{ $row['code'] }}</span></td>
                                            @foreach($pamuRanks as $pr)
                                                <td style="text-align:center;font-family:var(--font-sans);font-size:11px;{{ $row[$pr] > 0 ? 'font-weight:700;color:var(--text)' : 'color:var(--muted-2)' }}">{{ $row[$pr] ?: '—' }}</td>
                                            @endforeach
                                            <td style="text-align:right;font-weight:800;color:var(--accent);font-family:var(--font-sans);">{{ $row['total'] }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            <tfoot><tr>
                                <td style="font-weight:800">TOTAL</td>
                                @foreach($pamuRanks as $pr)<td style="font-family:var(--font-sans); text-align:center">{{ $pamuTotals[$pr] ?? 0 }}</td>@endforeach
                                <td style="font-family:var(--font-sans); text-align:right">{{ $pamuTotals['total'] ?? 0 }}</td>
                            </tr></tfoot>
                        </table></div></div>
                    </div>
        ...
        <div class="tab-pane" id="tab-afpos-rank">
            <div class="chart-grid-full" style="height:100%"><div class="card chart-card-tall">
                ...
                <div class="filter-row">
                    ...
                    <select id="afposRankFilter" onchange="updateAfposRankChart()"><option value="all">All AFPOS (Top 12)</option></select>
        ...
        <div class="tab-pane" id="tab-population">
            <div class="chart-grid-full" style="height:100%"><div class="card chart-card-tall">
                ...
                <div class="filter-row">
                    <span style="font-size:10px;color:var(--muted)"><strong>Click any bar to view the list of officers for that rank</strong></span>
                </div>
        ...
        <div class="tab-pane" id="tab-tenure">
            <div class="chart-grid-full" style="height:100%"><div class="card chart-card-tall">
                ...
                <div class="filter-row">
                    ...
                    <span style="font-size:10px;color:var(--muted);margin-left:8px">CPT 6yr · MAJ 6yr · LTC 7yr · COL 10yr — <strong>Click bars to see officers</strong></span>
                </div>
                <div class="card-bd" style="min-height:450px;padding:0">
                    <div class="chart-scroll-outer">
        ...
        <div class="tab-pane" id="tab-company-cdr">
            <div class="chart-grid-full" style="height:100%"><div class="card chart-card-tall">
                ...
                <div class="filter-row">
                    <label>View by:</label>
                    <select id="ccViewMode" onchange="updateCCChart()"><option value="rank">Group by Rank</option><option value="afpos">Group by AFPOS</option></select>
                    <span style="font-size:10px;color:var(--muted);margin-left:8px"><strong>Click bars to see officers</strong></span>
                </div>
                <div class="card-bd" style="min-height:450px;padding:0">
                    <div class="chart-scroll-outer">
        ...
        <div class="tab-pane" id="tab-cgsc">
            <div class="chart-grid-full" style="height:100%"><div class="card chart-card-tall">
                ...
                <div class="filter-row">
                    <label>View by:</label>
                    <select id="cgscViewMode" onchange="updateCGSCChart()"><option value="rank">Group by Rank</option><option value="afpos">Group by AFPOS</option></select>
                    <span style="font-size:10px;color:var(--muted);margin-left:8px"><strong>Click bars to see officers</strong></span>
                </div>
                <div class="card-bd" style="min-height:450px;padding:0">
                    <div class="chart-scroll-outer">
...
function initOverviewCharts() {
    ...
    new Chart(document.getElementById("rankChart"), {
        type: "bar",
        data: { labels: chartData.ranks, datasets: [
...
function initPopulationPyramid() {
    ...
    chartInstances['population'] = new Chart(document.getElementById('populationPyramid'), {
        type: 'bar',
        data: { labels: tab3Data.ranks, datasets: [{
