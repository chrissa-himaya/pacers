...
class SourcedatasController extends Controller
{
    ...
    public function create($id="0")
    {
        ...
        $rankpoints = Rankpoint::all()->keyBy('id');
        $data_items = [
            "data" => $sourcedata,
    ...
    public function show(Sourcedata $sourcedata)
    {
        ...
        $sourcedata->load('assignments.types', 'ranks', 'rankpoints');
        $columnHidden = array_merge($sourcedata->getDates(), ['id','assignment_id','type_id','rank_id','rankpoint_id']);
        $data_items = [
            "data" => $sourcedata,
    ...
    public function list(Request $request)
    {
        ...
        $data = $query->orderBy($orderColumn, $orderDir)
                      ->skip($start)
                      ->take($length)
                      ->with(['assignments', 'types', 'ranks', 'rankpoints'])
                    ->get();

        // Transform assignments so DataTables can display them
        $data = $data->map(function ($sourcedata) {
            $sourcedata->assignments_list = $sourcedata->assignments->pluck('name')->toArray();
            return $sourcedata;
        });
